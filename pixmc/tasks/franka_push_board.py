...
class FrankaPush(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.franka_default_dof_pos = to_torch(
            [1.157, -1.066, -0.155, -2.239, -1.841, 1.003, 0.469, 0.0, 0.0], device=self.device
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # Create table asset
        table_dims = gymapi.Vec3(0.7, 1.0, 0.4)
        asset_options = gymapi.AssetOptions()
        asset_options.fix_base_link = True
        ...
        asset_options = gymapi.AssetOptions()
        object_asset = self.gym.create_box(self.sim, self.object_size, self.object_size, self.object_size, asset_options)

        # Creat the avoidance box asset
        self.avoidance_box_size = 0.15
        avoidance_box_dims = gymapi.Vec3(0.1, 0.1, 0.005)
        asset_options = gymapi.AssetOptions()
        ...
        table_start_pose.p = gymapi.Vec3(0.3, 0.0, 0.5 * table_dims.z)

        avoidance_box_start_pose = gymapi.Transform()
        avoidance_box_start_pose.p = gymapi.Vec3(0.75, 0.1, 0.8)
        self.avoidance_box_start_position = avoidance_box_start_pose.p
        # define the avoid box vertices
        
        self.goal_y = 0.3

        object_start_pose = gymapi.Transform()
        object_start_pose.p = gymapi.Vec3(0.2, 0.0, table_dims.z + 0.5 * self.object_size)
    ...
    def post_physics_step(self):
        ...
        for i in range(self.num_envs):
            ...
            p1 = gymapi.Vec3(center_pos + self.avoidance_box_size/2, -self.avoidance_box_size/2, 0.86)
            p2 = gymapi.Vec3(center_pos - self.avoidance_box_size/2, -self.avoidance_box_size/2, 0.86)
            p3 = gymapi.Vec3(center_pos - self.avoidance_box_size/2, self.avoidance_box_size/2, 0.86)
            p4 = gymapi.Vec3(center_pos + self.avoidance_box_size/2, self.avoidance_box_size/2, 0.86)
...
def compute_franka_reward(
    ...
    rewards = -og_d - 0.0 * collision_penalty.squeeze() - 0.1 * lfo_d - 0.1 * rfo_d
