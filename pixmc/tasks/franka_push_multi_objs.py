...
class FrankaPushMulti(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        # Third object
        self.third_object_pos = self.root_state_tensor[:, self.env_third_object_ind, :3]

        # Fourth object
        self.fourth_object_pos = self.root_state_tensor[:, self.env_fourth_object_ind, :3]

        # Fifth object
        self.fifth_object_pos = self.root_state_tensor[:, self.env_fifth_object_ind, :3]

        # Sisth object
        self.sixth_object_pos = self.root_state_tensor[:, self.env_sixth_object_ind, :3]
        ...
        self.global_indices = torch.arange(
            self.num_envs * (1 + 1 + 1 + 1 + 1 + 1 + 1 + 1 + 1), dtype=torch.int32, device=self.device
        ...
        # Goal height diff
        self.red_to_goal = torch.zeros((self.num_envs, 1), dtype=torch.float, device=self.device)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        #asset_options.fix_base_link = True
        asset_options.density = 5000
        third_object_asset = self.gym.create_box(self.sim, third_object_dims.x, third_object_dims.y, third_object_dims.z, asset_options)
        ...
        #asset_options.fix_base_link = True
        asset_options.density = 5000
        sixth_object_asset = self.gym.create_box(self.sim, sixth_object_dims.x, sixth_object_dims.y, sixth_object_dims.z, asset_options)
        ...
        object_start_pose.p = gymapi.Vec3(0.65, 0.0, table_dims.z + 0.03) # this will be overwritten
        ...
        num_third_object_bodies = self.gym.get_asset_rigid_body_count(third_object_asset)
        num_third_object_shapes = self.gym.get_asset_rigid_shape_count(third_object_asset)
        num_fourth_object_bodies = self.gym.get_asset_rigid_body_count(fourth_object_asset)
        num_fourth_object_shapes = self.gym.get_asset_rigid_shape_count(fourth_object_asset)
        num_fifth_object_bodies = self.gym.get_asset_rigid_body_count(fifth_object_asset)
        num_fifth_object_shapes = self.gym.get_asset_rigid_shape_count(fifth_object_asset)
        num_sixth_object_bodies = self.gym.get_asset_rigid_body_count(sixth_object_asset)
        num_sixth_object_shapes = self.gym.get_asset_rigid_shape_count(sixth_object_asset)
        ...
        max_agg_bodies = num_franka_bodies + num_table_bodies + num_object_bodies + num_avoidance_box_bodies + num_third_object_bodies + num_fourth_object_bodies + num_fifth_object_bodies + num_sixth_object_bodies + num_goal_bodies
        max_agg_shapes = num_franka_shapes + num_table_shapes + num_object_shapes + num_avoidance_box_shapes + num_third_object_shapes + num_fourth_object_shapes + num_fifth_object_shapes + num_sixth_object_shapes + num_goal_shapes
        ...
        for i in range(self.num_envs):
            ...
            object_color = gymapi.Vec3(0, 1, 0) # this is the rgb value
            ...
            self.gym.set_rigid_body_color(env_ptr, third_object_actor, 0, gymapi.MESH_VISUAL_AND_COLLISION, gymapi.Vec3(1, 0, 0))
            ...
            self.gym.set_rigid_body_color(env_ptr, fifth_object_actor, 0, gymapi.MESH_VISUAL_AND_COLLISION, gymapi.Vec3(100, 159, 255) / 255)

            # Sixth object actor
            sixth_object_actor = self.gym.create_actor(env_ptr, sixth_object_asset, sixth_object_start_pose, "sixth_object", i, 0, 0)
            self.gym.set_rigid_body_color(env_ptr, sixth_object_actor, 0, gymapi.MESH_VISUAL_AND_COLLISION, gymapi.Vec3(100, 159, 255) / 255)
            ...
            self.third_objects.append(third_object_actor)
            self.fourth_objects.append(fourth_object_actor)
            self.fifth_objects.append(fifth_object_actor)
            self.sixth_objects.append(sixth_object_actor)
        ...
        self.env_third_object_ind = self.gym.get_actor_index(env_ptr, third_object_actor, gymapi.DOMAIN_ENV)

        self.env_fourth_object_ind = self.gym.get_actor_index(env_ptr, fourth_object_actor, gymapi.DOMAIN_ENV)

        self.env_fifth_object_ind = self.gym.get_actor_index(env_ptr, fifth_object_actor, gymapi.DOMAIN_ENV)

        self.env_sixth_object_ind = self.gym.get_actor_index(env_ptr, sixth_object_actor, gymapi.DOMAIN_ENV)
    ...
    def compute_reward(self, actions):
      self.rew_buf[:], self.reset_buf[:], self.successes[:], self.privilege_rew_buf[:], self.preference_rew_buf[:] = compute_franka_reward(
            ...
            self.lfinger_grasp_pos, self.rfinger_grasp_pos, self.object_pos, self.to_height, self.red_to_goal,
            ...
            self.objects_distance, self.avoidance_box_pos, self.third_object_pos, self.fourth_object_pos, self.fifth_object_pos, self.sixth_object_pos
        )
    ...
    def reset(self, env_ids):
        ...
        third_object_multi_env_ids_int32 = self.global_indices[env_ids, self.env_third_object_ind].flatten()

        fourth_object_multi_env_ids_int32 = self.global_indices[env_ids, self.env_fourth_object_ind].flatten()

        fifth_object_multi_env_ids_int32 = self.global_indices[env_ids, self.env_fifth_object_ind].flatten()

        sixth_object_multi_env_ids_int32 = self.global_indices[env_ids, self.env_sixth_object_ind].flatten()
        ...
        self.root_state_tensor[env_ids, self.env_avoidance_box_ind, 0] = self.avoidance_box_start_position.x + delta_x
        ...
        self.root_state_tensor[env_ids, self.env_third_object_ind, 0] = self.third_object_start_position.x + delta_x
        self.root_state_tensor[env_ids, self.env_third_object_ind, 1] = self.third_object_start_position.y + delta_y * 0.8
        self.root_state_tensor[env_ids, self.env_third_object_ind, 2] = self.object_z_init
        self.root_state_tensor[env_ids, self.env_third_object_ind, 3:6] = 0.0
        self.root_state_tensor[env_ids, self.env_third_object_ind, 6] = 1.0
        self.root_state_tensor[env_ids, self.env_third_object_ind, 7:10] = 0.0
        self.root_state_tensor[env_ids, self.env_third_object_ind, 10:13] = 0.0


        self.root_state_tensor[env_ids, self.env_fourth_object_ind, 0] = self.fourth_object_start_position.x + delta_x
        self.root_state_tensor[env_ids, self.env_fourth_object_ind, 1] = self.fourth_object_start_position.y + delta_y * 0.2
        self.root_state_tensor[env_ids, self.env_fourth_object_ind, 2] = self.object_z_init
        self.root_state_tensor[env_ids, self.env_fourth_object_ind, 3:6] = 0.0
        self.root_state_tensor[env_ids, self.env_fourth_object_ind, 6] = 1.0
        self.root_state_tensor[env_ids, self.env_fourth_object_ind, 7:10] = 0.0
        ...
        self.root_state_tensor[env_ids, self.env_fifth_object_ind, 7:10] = 0.0
        ...
        self.root_state_tensor[env_ids, self.env_sixth_object_ind, 7:10] = 0.0
        self.root_state_tensor[env_ids, self.env_sixth_object_ind, 10:13] = 0.0

        



        # connect object_multi_env_ids_int32 and avoidance_box_multi_env_ids_int32 and third_object_multi_env_ids_int32
        object_multi_env_ids_int32 = torch.cat((object_multi_env_ids_int32, avoidance_box_multi_env_ids_int32, third_object_multi_env_ids_int32, fourth_object_multi_env_ids_int32, fifth_object_multi_env_ids_int32, sixth_object_multi_env_ids_int32), dim=0)
    ...
    def compute_task_state(self):
        ...
        # distance to the goal region (min between 2 red objects)
        self.red_to_goal[:] = torch.min(torch.abs(self.third_object_pos[:, 0].unsqueeze(1) - self.goal_x), torch.abs(self.fourth_object_pos[:, 0].unsqueeze(1) - self.goal_x))
        self.red_to_goal[:] = torch.min(self.red_to_goal[:], torch.abs(self.fifth_object_pos[:, 0].unsqueeze(1) - self.goal_x))
        self.red_to_goal[:] = torch.min(self.red_to_goal[:], torch.abs(self.sixth_object_pos[:, 0].unsqueeze(1) - self.goal_x))
    ...
    def compute_oracle_obs(self):
        self.obs_buf[:] = torch.cat((
            ...
            self.third_object_pos, self.fourth_object_pos, self.fifth_object_pos, self.sixth_object_pos, self.red_to_goal
        ), dim=-1)
...
def compute_franka_reward(
    ...
    lfinger_grasp_pos: Tensor, rfinger_grasp_pos: Tensor, object_pos: Tensor, to_height: Tensor, red_to_goal: Tensor,
    ...
    objects_distance: Tensor, avoidance_box_pos: Tensor, third_object_pos: Tensor, fourth_object_pos: Tensor, fifth_object_pos: Tensor, sixth_object_pos: Tensor
    ...
    # disliked object to goal distance
    ...
    preference_rewards = og_d + disliked_og_dist_reward
