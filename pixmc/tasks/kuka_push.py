...
class KukaPush(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        max_agg_bodies = num_kuka_bodies + num_table_bodies + num_object_bodies + num_avoidance_box_bodies + num_goal_bodies
        max_agg_shapes = num_kuka_shapes + num_table_shapes + num_object_shapes + num_avoidance_box_shapes + num_goal_shapes
    ...
    def compute_reward(self, actions):
      self.rew_buf[:], self.reset_buf[:], self.successes[:], self.privilege_rew_buf[:], self.preference_rew_buf[:] = compute_kuka_reward(
            self.reset_buf, self.progress_buf, self.successes, self.actions,
            self.index_grasp_pos, self.middle_grasp_pos, self.ring_grasp_pos, self.thumb_grasp_pos, self.object_pos, self.to_height,
            self.object_z_init, self.thumb_dist_reward_scale, self.lift_bonus_reward_scale,
...
def compute_kuka_reward(
    ...
    index_grasp_pos: Tensor, middle_grasp_pos: Tensor, ring_grasp_pos: Tensor, thumb_grasp_pos: Tensor, object_pos: Tensor, to_height: Tensor,
    object_z_init: float, thumb_dist_reward_scale: float, lift_bonus_reward_scale: float,
    goal_dist_reward_scale: float, goal_bonus_reward_scale: float, action_penalty_scale: float,
    contact_forces: Tensor, rigid_body_arm_inds: Tensor, max_episode_length: int, collision_penalty: Tensor,
    ...
    rewards = -4.0 * preference_rewards - 3.0 * objects_distance.squeeze() -  0.5 * (io_d + to_d + mo_d + ro_d) - action_penalty_scale * action_penalty
    ...
    # Arm collision
    arm_collision = torch.any(torch.norm(contact_forces[:, rigid_body_arm_inds, :], dim=2) > 1.0, dim=1)
