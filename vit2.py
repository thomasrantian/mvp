...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def get_last_selfattention(self, x):
        B = x.shape[0]
        x = self.patch_embed(x) # B x total_patches x feat_dim
    ...
    def extract_encoded_patch_embed(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
        ...
        for blk in self.blocks:
            x = blk(x)
        x = x.detach().float() # B x len x feat_dim
        x = x[:, 1:].detach().float()
        return x
    ...
    def forward_classifier(self, x):
        return torch.sigmoid(self.head(self.forward_norm(self.extract_feat(x))))
